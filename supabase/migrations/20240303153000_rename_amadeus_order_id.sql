...
-- 1. Rename the column in flight_bookings if it exists
DO $$ 
BEGIN 
  IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='flight_bookings' AND column_name='amadeus_order_id') THEN
...
-- Then, add the new constraint including 'amadeus' to avoid breaking existing data
ALTER TABLE public.flight_bookings ADD CONSTRAINT flight_bookings_provider_check CHECK (provider IN ('amadeus', 'mystifly', 'mystifly_v2', 'duffel'));
...
-- Then, add the new constraint including 'amadeus' to avoid breaking existing data
ALTER TABLE public.booking_sessions ADD CONSTRAINT booking_sessions_provider_check CHECK (provider IN ('amadeus', 'mystifly', 'mystifly_v2', 'duffel'));
