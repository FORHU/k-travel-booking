...
export function BookingForSection({
    ...
    return (
        <div className="bg-white dark:bg-slate-900 rounded-lg sm:rounded-xl border border-slate-200 dark:border-white/10 p-2.5 sm:p-6 shadow-sm">
            <h2 className="text-sm sm:text-xl font-bold text-slate-900 dark:text-white mb-1.5 sm:mb-4">Who is the booking for?</h2>
            <div className="flex gap-1.5 sm:gap-4 mb-2 sm:mb-4">
                <button
                    className={`flex-1 py-1 sm:py-2 text-[11px] sm:text-sm rounded sm:rounded-lg border ${bookingFor === 'myself' ? 'border-blue-500 bg-blue-50 text-blue-700' : 'border-slate-200'}`}
                    onClick={() => onBookingForChange('myself')}
                >
                    Myself
                </button>
                <button
                    className={`flex-1 py-1 sm:py-2 text-[11px] sm:text-sm rounded sm:rounded-lg border ${bookingFor === 'someone_else' ? 'border-blue-500 bg-blue-50 text-blue-700' : 'border-slate-200'}`}
